...
case class Comparison(val left: Expression, val right: Expression, val typ: String, val pos: (Int, Int)) extends Expression
case class BinaryOperation(val left: Expression, val right: Expression, val typ: String, val pos: (Int, Int)) extends Expression
case class UnaryOperation(val right: Expression, val typ: String, val pos: (Int, Int)) extends Expression
...
case class Argument(val name: String, val typ: Type, val pos: (Int, Int)) extends Ast
...
case class Declaration(val names: List[DeclarationName], val typ: Type, val pos: (Int, Int)) extends Instruction
case class DeclarationName(val name: String, val value: Expression, val pos: (Int, Int)) extends Instruction


object ListHelper {
	def nil: List[Any] = Nil
	def add(value: Any, l: List[Any]): List[Any] = value::l
	def one(value: Any): List[Any] = value::Nil
};
